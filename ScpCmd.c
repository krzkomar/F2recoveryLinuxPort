...
void ScpA_PushConst( Intp_t *scr ) // 0x8001
{
    ...
    scr->CodePC += 4;
    SCP_DBGA( "  Const( [%x]%x )", scr->Opcode, addr );
}
...
void ScpA_If( Intp_t *scr ) // 802F ifcon( skip_addr, cond ) -> 'if( Cond == 0 ) then begin'
{
	...
	SCP_DBGA( "IF( [%x]%p == 0 ) GOTO [%x]%x", type[ 0 ], ptr, type[ 1 ], addr );
    } else {
...
void ScpA_SetLVar( Intp_t *scr ) // 8031 ?( arg1, arg2 ) set local var
{
    ...
    if( Type2 == SCR_PTR ){
	SCP_DBGA( "SetLVarp( [%x]%p, [%x]%x )", Type2, p, Type1, Arg1 );
	...
	SCP_DBGA( "SetLVari( [%x]%x, [%x]%x )", Type2, Arg2, Type1, Arg1 );    
...
void ScpA_8013( Intp_t *scr ) // 8013 push ?
{
	...
	SCP_DBGA( "?8013?( [%x]%p, [%x]%x )", Type2, p, Type1, Arg1 );
    }SCP_DBG_ELSE{
