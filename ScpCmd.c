...
/*
    name: Const
    dsc:  Copy constant value from code to argument stack
    code  0x8001, 0xC001, 0x9001, 0x9801, 0xA001
    ret:  none
*/
void ScpA_PushConst( Intp_t *scr ) // 0x8001
{
    SCP_DBG_VAR;
    uint32_t addr;
    ...
    scr->CodePC += 4;
    SCP_DBGA( "  Const( [%x]%i )", scr->Opcode, addr );
}
...
void ScpA_802B( Intp_t *scr ) // 802B push_base( ArgsCnt ) ?
{
    ...
    SCP_DBGA( "  PUSHA_BASE( [%x]%i base:%d )", type, ArgsCnt, scr->Base );
    SCP_PUSHARG_B( SCR_INT, scr->Base, scr );
    scr->Base = scr->StackApos - 10 * ArgsCnt;
}
...
void ScpA_Ret( Intp_t *scr ) // 802A 'Ret( ... )
{
    ...
    SCP_DBGA( "  LEAVE_FRM %i -> %i", scr->StackApos, scr->Base );
...
void ScpA_Dump( Intp_t *scr ) // 802E dump( arg_cnt, ... )
{
    ...
    SCP_DBGA( "DUMP( [%x]%i )", type, arg_cnt );
...
void ScpA_Call( Intp_t *scr ) // 8006 'call( procedure, procedure_time )
{
    ...
    SCP_DBGA( "CALL(( [%x]%i, [%x]%i )", type2, addr, type1, time );
...
void ScpA_CondCall( Intp_t *scr ) // 8007 cond_call( procedure, procedure_address )
{
    ...
    SCP_DBGA( "COND_CALL( [%x]%i, [%x]%i )", type2, addr, type1, ProcIdx );
...
void ScpA_Cancel( Intp_t *scr ) // 8048 'cancel( proc )'
{
    ...
    SCP_DBGA( "CANCEL( [%x]%i )", type, ProcOffs );
...
void ScpA_If( Intp_t *scr ) // 802F ifcon( skip_addr, cond ) -> 'if( Cond == 0 ) then begin'
{
    ...
    SCP_DBG_IF( type[ 0 ] == SCR_PTR ){
	SCP_DBGA( "IF( [%x]%p == 0 ) GOTO [%x]%i", type[ 0 ], ptr, type[ 1 ], addr );
    } else {
	SCP_DBGA( "IF( [%x]%i == 0 ) GOTO [%x]%i", type[ 0 ], Cond, type[ 1 ], addr );
    }
    if( type[ 0 ] == SCR_PTR ){
...
void ScpA_WhileDo( Intp_t *scr ) // 8030 while( Cond, SkipAddress ) -> 'while( Cond ) do begin'
{
	...
	SCP_DBGA( "WHILE( [%x]%i, [%x]%i )", type1, Cond, type2, BreakAddr );
    } else {
	SCP_DBGA( "WHILE( [%x]%i )", type1, Cond );
    }
...
/*
    Set local variable
*/
void ScpA_SetLVar( Intp_t *scr ) // 8031 ?( arg1, arg2 ) set local var
{
    SCP_DBG_VAR;
    uint16_t Type1, Type2;
    int Arg1, Arg2, k;
    ...
    SCP_GETARGX( Type2, Arg2, p, scr );
    k = IntpReadBei( scr->StackA, 10 * Arg1 + scr->Base );
    if( *(&scr->StackA[10 * Arg1 + 8] + scr->Base) == 0x0198 ) IntpStringDeRef( scr, SCR_FSTRING, k );
    if( Type2 == SCR_PTR ){
	SCP_DBGA( "SetLVarp( [%x]%p, [%x]%i )", Type2, p, Type1, Arg1 );
	IntpWriteBew( (uint64_t)p >> 48, scr->StackA, 10 * Arg1 + scr->Base );
	IntpWriteBew( (uint64_t)p >> 32, scr->StackA, 10 * Arg1 + scr->Base + 2 );
	IntpWriteBew( (uint64_t)p >> 16, scr->StackA, 10 * Arg1 + scr->Base + 4 );
	IntpWriteBew( (uint64_t)p, scr->StackA, 10 * Arg1 + scr->Base + 6 );
    } else {
	SCP_DBGA( "SetLVari( [%x]%i, [%x]%i )", Type2, Arg2, Type1, Arg1 );    
	IntpWriteBew( Arg2 >> 16, scr->StackA, 10 * Arg1 + scr->Base );
	IntpWriteBew( Arg2, scr->StackA, 10 * Arg1 + scr->Base + 2 );
    }
    stack = &scr->StackA[ 10 * Arg1 + 8 ] + scr->Base;
    stack[ 0 ] = Type2 >> 8; 
    stack[ 1 ] = Type2;
...
void ScpA_GetLVar( Intp_t *scr ) // 8032
{
    ...
    SCP_GETARG( Type1, Arg1, scr );
    SCP_DBGA( "GetLVar( [%x]%i )", Type1, Arg1 );
	...
	SCP_PUSHARG_AP( t, p, scr );
    } else {
	SCP_PUSHARG_A( t, k, scr );
    }    
}
...
void ScpA_CmpNE( Intp_t *scr ) // 8034 'stack:=p2<>p1'
{
    ...
    SCP_DBGA( "( [%x]%i != [%x]%i )", Type2, Arg2, Type1, Arg1 );
...
void ScpA_LogAnd( Intp_t *scr ) // 803E
{
	...
	case SCR_FSTRING:
	    switch( Type1 ){
    		...
    		case SCR_PTR:	  IntpPushIntStack( scr->StackA, &scr->StackApos, Arg1p != 0 ); break;
    	    }
    	    break;    
	case SCR_FLOAT:
	    switch( Type1 ){
    		...
    		case SCR_PTR:	  IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2f && Arg1p ); break;
	    }        
    	    break;        
	case SCR_INT:
	    switch( Type1 ){
		...
		case SCR_FSTRING: IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2 != 0 ); break;
    		...
    		case SCR_PTR:	  IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2 && Arg1p ); break;
	    }
	    break;
	case SCR_PTR:
	    switch( Type1 ){
		...
		case SCR_FSTRING: IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2 != 0 ); break;
    		case SCR_FLOAT:   IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2 && (Arg1f != 0.0) ); break;
    		case SCR_INT:     IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2 && Arg1 ); break;
    		case SCR_PTR:	  IntpPushIntStack( scr->StackA, &scr->StackApos, Arg2p && Arg1p ); break;
	    }	
	    break;
    }
    IntpPushwA( scr, SCR_INT );
...
void ScpA_CallExt( Intp_t *scr ) // 8005 call( adres ) or call( ?, adres )
{
	...
	SCP_DBGA( "CALL( [%x]%i )", Type, Arg );
        scr->CodePC = IntpReadBei( (char *)scr->ProcTable + 24 * Arg + 20, 0 ); // BodyOffst
        if( flags & INTP_P_CRITICAL ) scr->Flags |= SCR_FCRITICAL;
        return;
    }
    // Imported procedure
    flags = IntpReadBei( (char *)scr->ProcTable + 10 * Arg + 8, 0 );
    ExtName = scr->ProcVarNames + flags;
DD
printf("=>%p, %x\n", ExtName, flags );
printf("==>%s\n", ExtName );
    SCP_ERRTYPEF( !ExportGetProcedure( ExtName, &ArgV, &ArgC_expected ), stmp, "External procedure %s not found", ExtName );
    ...
    SCP_DBGA( "CALL_EXT( [%x]%i, %s )", Type1, ArgC_got, ExtName );
    // save context
    ...
    scr->Flags |= 0x20;
    scr->Flags = 0;
    i = IntpReadBei( &scr->ProcTable[ 10 * Arg ].NameOfst, 4 );
    scr->CodePC = ArgV;
printf("==>%x\n", ArgV);
...
void ScpA_ExitProc( Intp_t *scr ) // 801F ?( arg1, arg2, arg3 )
{
	...
	scr->Func = NULL;
	SCP_DBGA( "EXIT_PROC( [%x]%i )", Type2, Arg1 );
    } else {
...
void ScpA_8025( Intp_t *scr ) // 8025 O_POP_FLAGS_RETURN_VAL_EXIT
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "O_POP_FLAGS_RETURN_VAL_EXIT( [%x]%i )", Type, Arg );
    ScpA_ExitProc( scr );
    IntpPopwB( scr ); scr->CodePC = IntpPopiB( scr );
    scr->Flags |= 0x40;
...
void ScpA_8026( Intp_t *scr ) // 8026 O_POP_FLAGS_RETURN_VAL_EXIT_EXTERN
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "O_POP_FLAGS_RETURN_VAL_EXIT_EXTERN( [%x]%i )", Type, Arg );
    ScpA_ExitProc( scr );
...
void ScpA_8024( Intp_t *scr ) // 8024 O_POP_FLAGS_RETURN_VAL_EXTERN
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "O_POP_FLAGS_RETURN_VAL_EXTERN( [%x]%i )", Type, Arg );
    ScpA_ExitProc( scr );
    IntpPopwB( scr ); p = IntpPopPtrStack( scr->StackB, &scr->StackIdxB );
    IntpPopwB( scr ); p->Func = IntpPopPtrStack( scr->StackB, &scr->StackIdxB );
    IntpPopwB( scr ); p->Flags = IntpPopIntStack( scr->StackB, &scr->StackIdxB );
...
void ScpA_800C( Intp_t *scr ) // 800C
{
    ...
    SCP_DBGA( "  ADR To DAT -> [%x]%i", Type, Arg );
    IntpPushIntStack( scr->StackA, &scr->StackApos, Arg ); IntpPushwA( scr, Type );
}
...
void ScpA_800D( Intp_t *scr ) // 800D O_D_TO_A
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "  DAT To ADR -> [%x]%i", Type, Arg );
...
void ScpA_8012( Intp_t *scr ) // 8012 pop ?
{
    SCP_DBG_VAR;
    uint16_t Type;
    int Arg, val;
    ...
    SCP_GETARGX( Type, Arg, p, scr );
    SCP_DBGA( "POP( [%x]%i )",Type, Arg );
    Type = *( (uint16_t *)(&scr->StackA[ 10 * Arg + scr->SaveStack + 8 ]) );
    Type = ((Type & 0x00ff) << 8) | ( (Type & 0xff00) >> 8 );

    if( Type != SCR_INT && Type != SCR_FLOAT && (Type & ~0x800) != SCR_STRING ){
	p = IntpReadBep( scr->StackA, 10 * Arg + scr->SaveStack );
	SCP_PUSHARG_AP( Type, p, scr );	
    } else {
	val = IntpReadBei( scr->StackA, 10 * Arg + scr->SaveStack );
	SCP_PUSHARG_A( Type, val, scr );
    }
}

void ScpA_8013( Intp_t *scr ) // 8013 push ?
{
    SCP_DBG_VAR;
    unsigned int Arg1,Arg2, tmp, i;
    uint16_t Type1, Type2;
    void *p;
    
    SCP_GETARG( Type1, Arg1, scr );
    SCP_GETARGX( Type2, Arg2, p, scr );
    SCP_DBG_IF( Type2 == SCR_PTR ){
	SCP_DBGA( "?8013?( [%x]%p, [%x]%i )", Type2, p, Type1, Arg1 );
    }SCP_DBG_ELSE{
	SCP_DBGA( "?8013?( [%x]%i, [%x]%i )", Type2, Arg2, Type1, Arg1 );
    }
    tmp = IntpReadBei( scr->StackA, 10 * Arg1 + scr->SaveStack );
    if( *(uint16_t *)&scr->StackA[ 10 * Arg1 + 8 + scr->SaveStack ] == 0x0198 ) IntpStringDeRef( scr, SCR_FSTRING, tmp );

    // data
    if( Type2 == SCR_PTR ){ // pointer
	for( i = 0; i < sizeof( void * ) / 2; i++ )
	    IntpWriteBew( ((uint64_t)p) >> (48 - i*16), scr->StackA, 10 * Arg1 + scr->SaveStack + i*2 );
	// type
	scr->StackA[ 10 * Arg1 + 8 + scr->SaveStack + 0 ] = Type2 >> 8;
	scr->StackA[ 10 * Arg1 + 8 + scr->SaveStack + 1 ] = Type2;
    } else { // int
	IntpWriteBew( Arg2 >> 16, scr->StackA, 10 * Arg1 + scr->SaveStack );
	IntpWriteBew( Arg2, scr->StackA, 10 * Arg1 + scr->SaveStack + 2 );
	// type
	scr->StackA[ 10 * Arg1 + 8 + scr->SaveStack + 0 ] = Type2 >> 8;
	scr->StackA[ 10 * Arg1 + 8 + scr->SaveStack + 1 ] = Type2;
    }
...
void ScpA_8018( Intp_t *scr ) // 8018
{
    ...
    SCP_GETARG( Type1, Arg1, scr );
    SCP_GETARG( Type2, Arg2, scr );
    SCP_DBGA( "?8018?( [%x]%i, [%x]%i )", Type2, Arg2, Type1, Arg1 );
...
void ScpA_FetchAddr( Intp_t *scr ) // 802D
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "FETCH_PROC_ADDRESS( [%x]%i )", Type, Arg );
...
void ScpA_SetExportedVar( Intp_t *scr ) // 8015
{
    ...
    if( Type2 != SCR_PTR ){
	SCP_DBGA( "SET_EXPORTED_VAR: %s = {[%x]%i}", SCP_DNAME( Arg1 ), Type2, Arg2 );
...
void ScpA_GetExportedVar( Intp_t *scr ) // 8014
{
    ...
    uint16_t Type;
    char stmp[ 256 ];
    int Arg, tmp[ sizeof( void *) / sizeof( int ) ];
...
void ScpA_ExportProcedure( Intp_t *scr ) // 8017
{
    ...
    SCP_GETARG( Type1, Arg1, scr );
    SCP_GETARG( Type2, Arg2, scr );
    SCP_DBGA( "EXPORT_PROCEDURE: [%x]%i, [%x]%i )", Type2, Arg2, Type1, Arg1 );
    tmp = &scr->ProcVarNames[ IntpReadBei( &scr->ProcTable[ 10 * Arg1 ].NameOfst, 0 ) ];
    n = IntpReadBep( &scr->ProcTable[ 10 * Arg1 ].NameOfst, 16 );
...
void ScpA_CallStart( Intp_t *scr ) // 8008 callstart()
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "CALL_START( [%x]%i)", Type, Arg );
...
void ScpA_8027( Intp_t *scr ) // 8027 ?( arg1, arg2 )
{
    ...
    SCP_GETARG( Type1, Arg1, scr );
    SCP_GETARG( Type2, Arg2, scr );
    SCP_DBGA( "PROCEDURE[?]( [%x]%i, [%x]%i )", Type2, Arg2, Type1, Arg1 );
    if( IntpReadBei( &scr->ProcTable->NameOfst + 10 * Arg2, 20 ) != Arg1 ){
        sprintf( stmp, "Wrong number of args to procedure %s\n", IntpGetName( scr, IntpReadBei( &scr->ProcTable->NameOfst + 10 * Arg2, 0 ) ) );
        IntpError( stmp );
    }
}
...
void ScpA_LookupStringProc( Intp_t *scr ) // 8028
{
    ...
    SCP_GETARG( Type, Arg, scr );
    SCP_DBGA( "LOOKUP_STRING_PROC( [%x]%i )", Type, Arg );
...
void ScpAInitCommands()
{
    ...
    SciAddOpcode( 0x8031, ScpA_SetLVar );
    SciAddOpcode( 0x8032, ScpA_GetLVar );
